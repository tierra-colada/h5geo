...
#include "../h5core/h5core_enum.h"

#include <math.h>
#include <Eigen/Dense>

/*!
 * \brief The Deviation class
...
template<typename D, typename T>
Eigen::MatrixX<typename D::Scalar> TvdXY2MdAzimIncl(
    const Eigen::DenseBase<D> &M,
    const T& x0, const T& y0);

template<typename D>
Eigen::MatrixX<typename D::Scalar> TvdDxDy2MdAzimIncl(
    const Eigen::DenseBase<D> &M);

template<typename D, typename T>
Eigen::MatrixX<typename D::Scalar> TvdssXY2MdAzimIncl(
    const Eigen::DenseBase<D> &M,
    const T& x0, const T& y0, const T& kb);

template<typename D, typename T>
Eigen::MatrixX<typename D::Scalar> TvdssDxDy2MdAzimIncl(
    const Eigen::DenseBase<D> &M,
    const T& kb);
    ...
    const h5geo::AngleUnits& angleUnits)
{
  ...
  Eigen::MatrixX<typename D::Scalar> M_MdXYTvd =
      MdAzIncl2MdXYTvd(M, x0, y0, angleUnits);
    ...
    const T& x0, const T& y0, const T& kb)
{
  ...
  Eigen::MatrixX<typename D::Scalar> M_MdAzimIncl =
      TvdXY2MdAzimIncl(M, x0, y0);

  Eigen::MatrixX<typename D::Scalar> M_OUT(M.rows(), 9);
  M_OUT.col(0) = M_MdAzimIncl.col(0); // MD
  ...
  M_OUT.col(7) = M_MdAzimIncl.col(1); // AZ
  M_OUT.col(8) = M_MdAzimIncl.col(2); // INCL
  return M_OUT;
}
...
template<typename D, typename T>
Eigen::MatrixX<typename D::Scalar> h5geo::TvdXY2MdAzimIncl(
    const Eigen::DenseBase<D> &M,
    const T& x0, const T& y0)
{
  if (M.cols() != 3)
  ...
  for (size_t i = 0; i < M.rows(); i++){
    ...
    T dMD_dy = _dMDFromDeltaEast(dy, I1, I2, A1, A2, RF, h5geo::AngleUnits::RADIAN);
    T dMD_dx = _dMDFromDeltaNorth(dx, I1, I2, A1, A2, RF, h5geo::AngleUnits::RADIAN);
...
template<typename D>
Eigen::MatrixX<typename D::Scalar> h5geo::TvdDxDy2MdAzimIncl(
    const Eigen::DenseBase<D> &M)
{
  if (M.cols() != 3)
    return Eigen::MatrixX<typename D::Scalar>();
  ...
  for (size_t i = 0; i < M.rows(); i++){
    ...
    dMD = _dMDFromDeltaZ(dz, I1, I2, RF, h5geo::AngleUnits::RADIAN);
    typename D::Scalar dMD_dy = _dMDFromDeltaEast(dy, I1, I2, A1, A2, RF, h5geo::AngleUnits::RADIAN);
    typename D::Scalar dMD_dx = _dMDFromDeltaNorth(dx, I1, I2, A1, A2, RF, h5geo::AngleUnits::RADIAN);
...
template<typename D, typename T>
Eigen::MatrixX<typename D::Scalar> h5geo::TvdssXY2MdAzimIncl(
    const Eigen::DenseBase<D> &M,
    const T& x0, const T& y0, const T& kb)
{
  if (M.cols() != 3)
  ...
  for (size_t i = 0; i < M.rows(); i++){
    if (i == 0){
      dx = M(0, 1) - x0;
      dy = M(0, 2) - y0;
      dz = M(0, 0) - kb; // convert TVDSS to TVD
      A1 = atan2(dx, dy);
      A2 = A1;
      I1 = atan2(sqrt(pow(dx,2)+pow(dy,2)), dz);
      I2 = I1;
      ...
      dx = M(i, 1) - M(i-1, 1);
      dy = M(i, 2) - M(i-1, 2);
      dz = M(i, 0) - M(i-1, 0); // convert TVDSS to TVD
      A1 = M_OUT(i-1, 1);
      A2 = atan2(dx, dy);
      I1 = M_OUT(i-1, 2);
      I2 = atan2(sqrt(pow(dx,2)+pow(dy,2)), dz);
...
template<typename D, typename T>
Eigen::MatrixX<typename D::Scalar> h5geo::TvdssDxDy2MdAzimIncl(
    const Eigen::DenseBase<D> &M,
    const T& kb)
{
  if (M.cols() != 3)
