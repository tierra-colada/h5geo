...
h5geo::TxtEncoding H5SeisImpl::getSEGYTxtEncoding(
...
h5geo::SegyEndian H5SeisImpl::getSEGYEndian(
    const std::string& segy)
{
  std::ifstream file(segy);
  if (!file.is_open())
...
h5geo::SegyFormat H5SeisImpl::getSEGYFormat(
...
void H5SeisImpl::readSEGYTxtHdr(
    const std::string& segy,
...
void H5SeisImpl::readSEGYBinHdr(
    const std::string& segy,
    double binHdr[30], h5geo::SegyEndian endian)
{
  ...
  if (O32_HOST_ORDER == O32_LITTLE_ENDIAN){
    if (endian == h5geo::SegyEndian::Big){
      bswap<int>(std::begin(binHdr4), std::end(binHdr4), std::begin(binHdr4));
      bswap<short>(std::begin(binHdr2), std::end(binHdr2), std::begin(binHdr2));
    }
  } else if (O32_HOST_ORDER == O32_BIG_ENDIAN){
...
double H5SeisImpl::getSEGYSampRate(
    const std::string& segy, h5geo::SegyEndian endian)
{
  double binHdr[30];
...
size_t H5SeisImpl::getSEGYNSamp(
    const std::string& segy, h5geo::SegyEndian endian)
{
  double binHdr[30];
  readSEGYBinHdr(segy, binHdr, endian);
