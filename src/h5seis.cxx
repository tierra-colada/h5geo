#include "../include/h5seis/h5seis.h"
#include "../include/h5seis/h5seiscontainer.h"

H5Seis::H5Seis(const h5gt::Group &group) :
  H5BaseObject(group),
...
std::optional<H5Seis>
H5Seis::create(h5gt::Group &group){
...
bool H5Seis::writeTextHeader(const char (&txtHdr)[40][80]){
...
bool H5Seis::writeBinHeader(const double (&binHdr)[30])
{
...
Eigen::VectorXd H5Seis::getBinHeader()
{
  auto opt = getBinHeaderD();
  if (!opt.has_value())
...
Eigen::MatrixXf H5Seis::getTrace(
...
Eigen::MatrixXd H5Seis::getTraceHeader(
    const size_t& fromTrc,
    size_t nTrc,
...
Eigen::VectorXd H5Seis::getTraceHeader(
    const std::string& hdrName,
    const size_t& fromTrc,
    const size_t& nTrc)
{
...
Eigen::VectorX<size_t> H5Seis::getSortedData(
...
ptrdiff_t H5Seis::getBinHeaderIndex(
    const std::string& hdrName)
{
  auto opt = getBinHeaderD();
  if (!opt.has_value())
...
ptrdiff_t H5Seis::getTraceHeaderIndex(
    const std::string& hdrName)
{
...
Eigen::VectorX<size_t> H5Seis::getTracePKeyIndexes(
...
Eigen::VectorXd H5Seis::getSamples(const size_t& trcInd){
...
double H5Seis::getFirstSample(const size_t& trcInd){
...
std::vector<std::string> H5Seis::getPKeyNames(){
...
Eigen::VectorXd H5Seis::getTraceHeaderMin(){
...
Eigen::VectorXd H5Seis::getTraceHeaderMax(){
...
double H5Seis::getTraceHeaderMin(
    const std::string& hdrName)
{
...
double H5Seis::getTraceHeaderMax(
    const std::string& hdrName)
{
  int ind = getTraceHeaderIndex(hdrName);
...
Eigen::MatrixX2d H5Seis::calcBoundaryStk2D(){
...
Eigen::MatrixX2d H5Seis::calcConvexHullBoundary(){
...
bool H5Seis::calcBinOriginOrientation3DStk(
...
bool H5Seis::calcAndWriteBoundary(){
  auto opt = getBoundaryD();
  if (!opt.has_value())
    return false;
...
bool H5Seis::calcAndWriteTraceHeaderLimits(
...
bool H5Seis::checkTraceHeaderLimits(
...
h5geo::SeisDataType H5Seis::getDataType(){
...
h5geo::SurveyType H5Seis::getSurveyType(){
...
bool H5Seis::hasPKeySort(const std::string& pKeyName)
{
  auto optUValG = getUValG();
  if (!optUValG.has_value())
    return false;
...
bool H5Seis::removePKeySort(const std::string& pKeyName){
  auto optUValG = getUValG();
  if (!optUValG.has_value())
    return false;
...
bool H5Seis::addPKeySort(const std::string& pKeyName){
  auto optUValG = getUValG();
  if (!optUValG.has_value())
    return false;
...
H5Seis::getSeisContainer(){
  h5gt::File file = getH5File();
  return H5SeisContainer::create(file);
}
